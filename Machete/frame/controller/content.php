...
class ContentController extends BuildQuery {
	...
	if(!empty(self::$db_content_row[DB_CONTENT_PDF])) {
	self::$db_content_formats[$controller.'/pdf'] = $params['id'];
	}
	if(!empty(self::$db_content_row[DB_CONTENT_VIDEO])) {
	self::$db_content_formats[$controller.'/video'] = $params['id'];
	}
	}
	}
