...
class View extends BuildHTML {
	...
	$form_attributes = array("class"=>"search_form grid_3","name"=>"search", "method"=>"get");
	...
	$menu_attributes = array("class"=>'normalmenu grid_5 '.$menu_class);
	...
	$a_end = BuildHTML::end_element(self::$a);
	echo $menu_start;
	echo $ul_start;
	...
	$a_attributes = array("href"=>'?node='.$parent_link, "title"=>$parent_desc);
	$li_attributes = array("id"=>$parent_id);
	$li_start = BuildHTML::start_element(self::$li, $li_attributes);
	$a_parent_start = BuildHTML::start_element(self::$a, $a_attributes);
	//can be abstracted using BuildHTML start element -- DONE AET 2014
	echo $li_start.$a_parent_start.$parent_name.$a_end;
	if (is_array($val)) {
	echo $ul_start;
	...
	$li_attributes = array("id"=>$child_id);
	$a_attributes = array("href"=>'?node='.$child_link, "title"=>$child_desc);
	$li_start = BuildHTML::start_element(self::$li, $li_attributes);
	$a_child_start = BuildHTML::start_element(self::$a, $a_attributes);
	//can be abstracted using BuildHTML start element -- ATTEMPTED/DONE AET 2014
	echo $li_start.$a_child_start.$child_name.$a_end.$li_end;
	}
	}
	echo $ul_end.$li_end;
	}
	...
	$related_attributes = array("class"=>'related_links grid_3 vertical_menu');
	//It's Grid 12 so it doesn't interact/screw up Grids 1 to 11, thank God -- AET
	...
	function draw_head($css, $js) {
		...
		// Fun fact - on my old Nexus 7, the device is bugged in some way that overlooks this flag.
		// It can and will pan and tilt, though not consistently.
	...
	function draw_footer() {
		...
		$content = 'Not-Quite &copy; Adam Tenhouse 2014';
